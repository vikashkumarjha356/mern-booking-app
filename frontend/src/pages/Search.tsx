...
const Search = () => {
    ...
    const facilityTypesChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        const facilityType = event.target.value;
        setSelectedFacilities((prevFacilityType) => event.target.checked ?
            [...prevFacilityType, facilityType] : prevFacilityType.filter((facility) => facility !== facilityType));
    }
        ...
        <div className='grid grid-cols-1 lg:grid-cols-[250px_1fr] gap-5'>
            <div className='rounded-lg border border-slate-300 p-5 h-fit sticky top-10'>
                <div className="space-y-5">
                    <h3 className="text-lg font-semibold border-b boder-slate-300 pb-5">
                        Filter by:
                    </h3>
                    <StarRatingFilter selectedStars={selectedStars} onChange={handleStarsChange} />
                    <HotelTypesFilter selectedHotelTypes={selectedHotelTypes} onChange={handleHotelTypeChange} />
                    <FacilitiesFilter selectedFacilities={selectedFacilities} onChange={facilityTypesChange} />
